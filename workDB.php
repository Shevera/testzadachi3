...
function checkPostParams(){
    ...
    $myFile         = isset($_FILES['fileName'])  ? $_FILES['fileName']  : null;
...
function checkFile($myFile){
    ...
    if($myFile['fileName']['size'] > 500000 && $flag !== true){
        return $myFile = null;
    }
    ...
    if ($flag && (copy($_FILES['fileName']['tmp_name'], $uploadfile)))
    {
        echo "<h3>Файл успешно загружен на сервер</h3>";
        return $myFile = $_FILES['fileName']['name'];
    }else{
